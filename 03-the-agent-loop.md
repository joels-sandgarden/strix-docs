...
The shared sandbox preserves one isolation boundary for the scan, while the SQLite memory keeps per agent state across cycles. That split lets Strix hold on to coordinator state and agent memory across multiple runs without giving up the single sandbox boundary that the whole tree shares.
