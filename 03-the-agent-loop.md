...
This matters because the SDK handles the model, tool, and result loop, while Strix carries the run-wide policy around it. One scan reuses a single sandbox session bundle, one coordinator, one budget ledger, and one set of lifecycle rules. That arrangement keeps the model loop stable and lets Strix change policy without changing how the SDK executes a turn. For the wider map, see [The Big Picture](./00-the-big-picture.md), [Anatomy of a Scan](./01-anatomy-of-a-scan.md), and [The Graph of Agents](./02-the-graph-of-agents.md).
