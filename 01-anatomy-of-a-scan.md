...
`strix` turns a command into a live scan by validating the environment, checking that Docker is ready, and making sure the sandbox image exists before anything else starts. From there it hands off to either the interactive TUI or the headless runner, depending on how the scan is launched.
